 ...
 */
const sendWebhook = async (moduleName, data) => {
    const clientUrlsStr = env_1.env.WEBHOOK_CLIENT_URL || "";
    const apiKey = env_1.env.API_KEY;
    ...
    const promises = clientUrls.map(async (clientUrl) => {
        ...
        try {
            ...
            if (!response.ok) {
                console.error(`[Webhook Error] Failed to send to ${endpoint}. Status: ${response.status} - ${response.statusText}`);
                console.error(`[Webhook Error] Response body:`, responseBody);
            }
            else {
                console.log(`[Webhook Success] Sent to ${endpoint}. Status: ${response.status}`);
            }
        }
        catch (error) {
            console.error(`[Webhook Error] Failed to send to ${endpoint}:`, error);
        }
    });
