...
import { StudentService, studentService } from "@/modules/student/service";
...
export class StudentController {
  ...
  getStatistic = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      sendResponse(res, 200, "Success fetch student statistics", data);
    } catch (error: unknown) {
      next(error instanceof Error ? error : new Error(String(error)));
    }
  };
  ...
  bulkCreate = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      sendResponse(res, 201, "Data berhasil ditambahkan", data);
    } catch (error: unknown) {
      next(error instanceof Error ? error : new Error(String(error)));
    }
  };
    ...
    try {
      if (!req.file)
        throw new BadRequestError(
          'No file uploaded. Please upload an Excel file with field name "file".',
        );
