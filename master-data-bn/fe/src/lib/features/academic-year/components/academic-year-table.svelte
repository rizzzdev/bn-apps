<script lang="ts">
	import { Button, Icon, Badge, Checkbox } from '$lib/components/atoms';
	...
	const exportData = () => {
		...
		const rows = filteredAcademicYears.map((item, i) => {
			...
			return [i + 1, a.code, a.startYear, a.endYear, a.status];
		});
	...
	async function handleSave() {
		try {
			...
			if (isCreateOpen) {
				const res = await apiClient('/academic-years', { method: 'POST', body: JSON.stringify(payload) });
				...
				const res = await apiClient(`/academic-years/${selectedAcademicYear.id}`, { method: 'PUT', body: JSON.stringify(payload) });
				const result = await res.json();
				if (!result.error) {
	...
	async function handleDelete() {
		try {
			...
			const res = await apiClient(`/academic-years/${selectedAcademicYear.id}`, { method: 'DELETE' });
			const result = await res.json();
			if (!result.error) {
	...
	async function handleBulkDelete() {
		try {
			const res = await apiClient('/academic-years/bulk', {
				method: 'DELETE',
	...
	const fetchAcademicYears = async (page: number, limitPerPage: number) => {
		isLoading = true;
		try {
			const res = await apiClient(`/academic-years?page=${page}&limit=${limitPerPage}&includeSemesters=true`);
			const result = await res.json();
	...
	let filteredAcademicYears = $derived(academicYears.filter(y => {
...
<div class="mb-md flex flex-col md:flex-row gap-sm items-center justify-between">
	<div class="flex flex-col md:flex-row gap-sm w-full md:w-auto h-auto">
		<SearchBar bind:value={searchQuery} placeholder="Cari kode atau tahun..." class="w-full md:w-80 h-10" />
	</div>
</div>
...
<Table class="mb-0">
	<TableHead>
		<TableRow header>
			<TableHeadCell width="w-12" align="center">
				<Checkbox checked={selectedIds.length === filteredAcademicYears.length && filteredAcademicYears.length > 0} onchange={toggleAll} />
			</TableHeadCell>
	...
	<TableBody>
		{#each filteredAcademicYears as year, i}
			<TableRow striped={i % 2 !== 0}>
				<TableCell align="center">
					<Checkbox bind:group={selectedIds} value={year.id} />
				</TableCell>
				<TableCell class="font-body-bold text-body-bold">{year.code}</TableCell>
				...
				<TableCell align="center">
					<div class="flex justify-center gap-xs">
						<button onclick={() => openSemesterModal(year)} title="Kelola Semester" class="h-8 w-8 border-3 rounded-lg border-on-background bg-primary-container text-primary flex items-center justify-center hover:bg-primary hover:text-on-primary transition-colors cursor-pointer">
							<Icon name="event_note" class="text-base" fill={0} />
						</button>
						<button onclick={() => openEdit(year)} title="Edit" class="h-8 w-8 border-3 rounded-lg border-on-background bg-surface-container-highest flex items-center justify-center hover:bg-secondary hover:text-on-secondary transition-colors cursor-pointer">
							<Icon name="edit" class="text-base" fill={0} />
						</button>
						<button onclick={() => openDelete(year)} title="Hapus" class="h-8 w-8 border-3 rounded-lg border-on-background bg-error-container text-error flex items-center justify-center hover:bg-error hover:text-on-error transition-colors cursor-pointer">
...
<Modal bind:isOpen={isCreateOpen} title="Tambah Tahun Ajaran">
	<div class="flex flex-col gap-sm">
		<div class="flex gap-sm">
			<FormField id="startYear-create" label="Tahun Mulai" bind:value={formData.startYear} placeholder="2024" />
			<FormField id="endYear-create" label="Tahun Selesai" bind:value={formData.endYear} placeholder="2025" />
		</div>
...
<Modal bind:isOpen={isEditOpen} title="Edit Tahun Ajaran">
	<div class="flex flex-col gap-sm">
		<div class="flex gap-sm">
			<FormField id="startYear-edit" label="Tahun Mulai" bind:value={formData.startYear} placeholder="2024" />
			<FormField id="endYear-edit" label="Tahun Selesai" bind:value={formData.endYear} placeholder="2025" />
		</div>
