...
{#if isOpen}
	...
	>
		<div
			...
			aria-modal="true"
		>
			<div class="border-on-background p-md flex items-center justify-between border-b-3 bg-primary-container">
				{#if title}
			...
			{#if footer}
				<div class="border-on-background p-md bg-surface-variant flex items-center justify-end border-t-3 gap-sm">
					{@render footer()}
				</div>
			{/if}
		</div>
