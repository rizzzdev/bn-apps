<script lang="ts">
	...
	function getCurrentSrc(): string | null {
		...
		if (currentImageUrl) return `${PUBLIC_API_URL}/attachments/file/${currentImageUrl}`;
		return null;
	}
...
<div class="flex flex-col items-center gap-sm py-sm px-md border-2 border-dashed border-on-background/30 rounded-xl bg-surface-container-highest/30">
	{#if label}
	...
	{#if getCurrentSrc()}
		...
		/>
	{:else if isDeleted}
		<div class="h-32 w-32 border-3 border-error/50 rounded-xl flex items-center justify-center bg-error-container">
			<Icon name="no_photography" class="text-4xl text-error" fill={1} />
		</div>
	{:else}
		<div class="h-32 w-32 border-3 border-on-background/30 rounded-xl flex items-center justify-center bg-surface-container-highest">
