...
export class ShadowSyncService {
  ...
  /**
   * Sync master students (status Aktif) to ShadowStudent table.
   */
    ...
    try {
      ...
      if (activeIds.length > 0) {
        await prisma.shadowStudent.updateMany({
          where: { id: { notIn: activeIds }, deletedAt: null },
          data: { deletedAt: now },
        }).catch(() => {});
      }
  ...
  /**
   * Sync master teachers (status Aktif) to ShadowTeacher table.
  ...
  async syncTeachers(): Promise<number> {
    try {
      ...
      for (const t of masterTeachers) {
        ...
        const prefix = t.prefixTitle?.trim() ? `${t.prefixTitle.trim()} ` : '';
        const suffix = t.suffixTitle?.trim() ? `, ${t.suffixTitle.trim()}` : '';
        const fullnameWithTitle = `${prefix}${t.fullname?.trim() ?? ''}${suffix}`;
