...
import { ChatRepository } from "#exam/modules/chat/index.js";
...
async function saveAdminNotifications(
  ...
  try {
    ...
    await notifyUsers(prisma, adminIds, type, title, message, meta);
  } catch {
...
async function saveSupervisorAndAdminNotifications(
  ...
  try {
    const [adminIds, supervisors] = await Promise.all([
      getAdminUserIds(),
      prisma.examSupervisor.findMany({ where: { examRoomId }, select: { userId: true } }),
    ]);
    const userIds = [...new Set([...adminIds, ...supervisors.map((s) => s.userId)])];
...
async function saveDirectNotification(
  ...
  try {
    const title = senderRole === "teacher" ? "Pesan dari Pengawas" : "Pesan dari Admin";
...
// ─── User info helpers (no User table — resolved from shadow DB + sentri) ───

type UserInfo = { username: string; fullname: string; role: string };

async function resolveUserInfo(userId: string): Promise<UserInfo | null> {
  const [teacher, student] = await Promise.all([
    prisma.shadowTeacher.findFirst({ where: { userId, deletedAt: null } }),
    prisma.shadowStudent.findFirst({ where: { userId, deletedAt: null } }),
  ]);
  if (teacher) {
    return { username: teacher.fullname, fullname: teacher.fullname, role: "teacher" };
  }
  if (student) {
    return { username: student.fullname, fullname: student.fullname, role: "student" };
  }
  return null;
}

async function resolveUserInfos(userIds: string[]): Promise<Map<string, UserInfo>> {
  const map = new Map<string, UserInfo>();
  if (userIds.length === 0) return map;
  const [teachers, students] = await Promise.all([
    prisma.shadowTeacher.findMany({ where: { userId: { in: userIds }, deletedAt: null } }),
    prisma.shadowStudent.findMany({ where: { userId: { in: userIds }, deletedAt: null } }),
  ]);
  for (const t of teachers) {
    map.set(t.userId, { username: t.fullname, fullname: t.fullname, role: "teacher" });
  }
  for (const s of students) {
    map.set(s.userId, { username: s.fullname, fullname: s.fullname, role: "student" });
  }
  return map;
}
...
export const getRoomParticipantStatuses = async (
  ...
  const statuses: ParticipantStatus[] = await Promise.all(
    participants.map(async (p) => {
      const info = infoMap.get(p.userId) ?? { username: p.userId, fullname: p.userId };
...
export const handleViolation = async (
  ...
  const info = (await resolveUserInfo(userId)) ?? { fullname: userId };
...
export const handleUnlock = async (
  ...
  const participantInfo = (await resolveUserInfo(participantUserId)) ?? {
    fullname: participantUserId,
  };

  await saveLog(
    examRoomId,
    "system",
    `${participantInfo.fullname} dibuka kuncinya oleh ${supervisorFullname}.`,
    supervisorId,
  );
  logger.info(
...
export const handleWarn = async (
  ...
  try {
    ...
    const targetInfo = (await resolveUserInfo(targetUserId)) ?? { fullname: targetUserId };
    ...
    await saveLog(
      examRoomId,
      "warning",
      `Peringatan dikirim ke ${targetInfo.fullname} oleh ${supervisorFullname}: "${message}"`,
      supervisorId,
    );
...
const CHAT_ROLES = new Set(["super_admin", "teacher"]);
...
export const handleChatSend = async (
  ...
  try {
    ...
    // Resolve receiver: supervisor from shadow DB, or admin from sentri.
    const receiverInfo = await resolveUserInfo(receiverId);
    let receiverRole: string | null = null;
    let receiverName = receiverId;

    if (receiverInfo && receiverInfo.role === "teacher") {
      receiverRole = "teacher";
      receiverName = receiverInfo.fullname;
    } else {
      try {
        const admins = await getOrchestrator().authData.findAllByRoles(["super_admin"]);
        const admin = admins.find((a) => a.id === receiverId);
        if (admin) {
          receiverRole = "super_admin";
          receiverName = admin.email ?? receiverId;
        }
      } catch {
        receiverRole = null;
      }
    ...
    if (!receiverRole || !CHAT_ROLES.has(receiverRole) || receiverRole === senderRole) {
      socket.emit("exam:error", {
    ...
    const senderInfo = (await resolveUserInfo(senderId)) ?? { fullname: senderId };
    const senderName = senderRole === "super_admin" ? (await getAdminSenderName(senderId)) : senderInfo.fullname;
    ...
    void saveDirectNotification(receiverId, senderName, senderRole, message.trim());
...
async function getAdminSenderName(userId: string): Promise<string> {
  try {
    const admins = await getOrchestrator().authData.findAllByRoles(["super_admin"]);
