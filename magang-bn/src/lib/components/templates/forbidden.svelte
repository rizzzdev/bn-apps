...
<div class="min-h-dvh bg-surface flex flex-col items-center justify-center p-4">
	<div class="max-w-sm w-full bg-surface border-2 border-on-background shadow-neo-sm p-5 flex flex-col items-center text-center gap-4">
		...
		<div class="flex flex-col gap-2">
			<h1 class="font-headline font-black text-base uppercase tracking-tight text-on-background">403</h1>
		...
		<div class="w-full flex flex-col gap-2 mt-3">
			<a
				href={portalUrl}
				class="w-full py-2 border-2 border-on-background bg-secondary text-surface font-bold uppercase tracking-widest shadow-neo-sm hover:translate-x-px hover:translate-y-px hover:shadow-none transition-all flex items-center justify-center gap-1"
			>
				<Icon name="apps" class="text-[10px]" />
				Kembali ke Portal
			</a>
			{#if user && user.roles}
			...
			<button onclick={logout} class="w-full py-2 border-2 border-on-background bg-surface text-error font-bold uppercase tracking-widest hover:bg-error hover:text-surface transition-colors flex items-center justify-center gap-1 cursor-pointer">
