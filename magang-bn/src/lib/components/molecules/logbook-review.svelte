<script lang="ts">
	...
	let { variant = "teacher" as "teacher" | "mentor" } = $props<{
	...
	async function fetchFiltersData() {
		...
		const res = await apiClient(
			`/internship-placements?${idParam}=${roleId}&limit=1000`,
		);
		if (res && !res.error) {
	...
	async function submitReview() {
		...
		const res = await apiClient(
			`/daily-logbooks/${selectedLogbook.id}/reviews`,
			{
				method: "POST",
				body: JSON.stringify({
					action: actionType,
...
>
	<SearchFilter
		...
		placeholder="Cari Judul / Isi Logbook..."
	/>
...
{#if loading}
	<div class="flex justify-center py-12">
		<Icon name="refresh" class="animate-spin text-lg text-primary" />
	</div>
{:else if logbooks.length === 0}
	<div
		class="border-2 border-on-background bg-surface shadow-neo-sm p-8 text-center animate-fade-in-up"
	>
		<Icon name="menu_book" class="text-2xl text-secondary mb-3" />
		<h3 class="font-headline text-base font-black uppercase mb-1">
			Belum Ada Logbook
		</h3>
		<p class="font-mono text-secondary mb-4">
			Belum ada logbook dari murid bimbingan yang perlu divalidasi.
		</p>
	</div>
{:else}
	<div class="space-y-6">
...
<Modal bind:show={showModal} title="Ajukan Revisi">
	...
	>
		<div class="flex flex-col space-y-2">
			<label
				for="reviewContent"
				class="font-headline font-black uppercase text-sm"
			...
			<textarea
				...
				required
				placeholder="Tuliskan bagian mana yang perlu diperbaiki oleh murid..."
				class="border-2 border-on-background p-2 text-[10px] focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all font-mono w-full"
			></textarea>
		</div>
		...
		<div class="flex gap-4">
			...
			>
				<Icon name={isSubmitting ? "hourglass_empty" : "send"} class="text-[10px]" />
