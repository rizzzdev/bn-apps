	...
	async function fetchStudents(pageNo = 1) {
		...
		if (teacherId) {
			...
			const res = await apiClient(
				`/internship-placements?teacherId=${teacherId}&page=${pageNo}&limit=10${searchParam}`,
			);
			if (res && !res.error) {
				students = res.data || [];
...
<svelte:head>
	<title>Murid Bimbingan | Magang-BN</title>
</svelte:head>
...
<div class="mb-6 animate-fade-in-up">
	...
	<p class="font-mono text-secondary text-[10px] mt-1">
		Daftar murid magang di bawah bimbingan Anda.
	</p>
</div>
