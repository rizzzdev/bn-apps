<script lang="ts">
	...
	let fullName = $derived(
		(formatFullName(data.profileData) || "MENTOR INDUSTRI").toUpperCase(),
	);
	...
	async function fetchDashboard() {
		...
		const todayStr = new Date()
			...
			.slice(0, 10);
		let url = `/dashboard/mentor?date=${todayStr}`;
...
<svelte:head>
	<title>Mentor Dashboard | Magang-BN</title>
</svelte:head>
...
>
	...
	/>
	<MetricCard
		title="Sertifikat Diunggah"
		value={fmt(s.uploadedCertificates)}
		icon="upload_file"
		variant="error"
	/>
</div>
