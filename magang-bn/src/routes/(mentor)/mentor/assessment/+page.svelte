<script lang="ts">
	...
	async function fetchParticipants(pageNo = 1) {
		loading = true;
		if (mentorId) {
			let url = `/internship-placements?industryMentorId=${mentorId}&page=${pageNo}&limit=10`;
	...
	async function fetchStudents() {
		if (mentorId) {
			const res = await apiClient(
				`/internship-placements?mentorId=${mentorId}&limit=1000`,
			);
	...
	$effect(() => {
		if (mentorId) {
			const _ =
				searchQuery + selectedStatus + selectedStudent + currentPage;
...
<svelte:head>
	<title>Penilaian Praktek | Magang-BN</title>
</svelte:head>
...
<div class="mb-6 animate-fade-in-up">
	...
	<p class="font-mono text-secondary text-[10px] mt-1">
		Berikan evaluasi terhadap performa teknis dan sikap peserta selama
		magang di perusahaan Anda.
	</p>
</div>
...
<UploadCertificateModal
	...
	onsubmit={async (file) => {
		...
		const uploadRes = await apiClient("/attachments", {
			method: "POST",
			body: formData,
		});
		if (uploadRes && uploadRes.data && uploadRes.data.url) {
			...
			if (!updateRes || updateRes.error)
				throw new Error(
					updateRes?.message || "Gagal menyimpan link sertifikat",
				);
