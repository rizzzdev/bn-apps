<script lang="ts">
	...
	async function fetchParticipants(pageNo = 1) {
		loading = true;
		let url = `/internship-placements?page=${pageNo}&limit=10`;
	...
	async function fetchStudents() {
		const res = await apiClient(`/internship-placements?limit=1000`);
		if (res && !res.error && res.data) {
			const unique = Array.from(
				new Map(
					res.data.map((p: any) => [p.student?.id, p.student?.name]),
				).entries(),
			).map(([id, name]) => ({
