<script lang="ts">
	...
	import { formatFullName, formatDateRange, formatBytesToMB } from "$lib/utils/helpers";
	...
	async function handleSubmit(e: Event) {
		...
		const res = await apiClient("/daily-logbooks", {
			method: "POST",
			body: formData,
		});
...
<svelte:head>
	<title>Isi Logbook | Magang-BN</title>
</svelte:head>
	...
	<div class="flex justify-between items-center">
		<div>
			...
			<p class="font-mono text-secondary text-[10px] mt-1">
				Catat aktivitas magangmu hari ini dengan detail.
			</p>
		</div>
	...
	<form onsubmit={handleSubmit} class="space-y-4">
		...
		<div class="flex flex-col space-y-2">
			...
			<textarea
				...
				class="border-2 border-on-background bg-surface p-2 font-mono text-[10px] text-on-background focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all shadow-neo-sm w-full"
				placeholder="Ceritakan detail aktivitas magang hari ini..."
			></textarea>
		</div>
