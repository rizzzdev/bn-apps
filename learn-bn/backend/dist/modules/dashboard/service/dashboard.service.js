...
export class DashboardService {
    ...
    async getTeacherPendingGrading(teacherId) {
        if (!teacherId) {
    ...
    async getStudentPendingItems(studentId) {
        if (!studentId) {
        ...
        const [materials, assignments, quizzes, unreadCount, assignCount, quizCount, submittedAssignments, totalAssignments, readMaterials, totalMaterials, doneQuizzes, totalQuizzes] = await Promise.all([
        ...
        return {
            ...
            materials: materials.map((m) => ({
                id: m.id,
                title: m.title,
                className: m.classes?.map((c) => c.class?.name).filter(Boolean).join(', ') || 'Kelas',
            ...
            quizzes: quizzes.map((q) => ({
                id: q.id,
                title: q.title,
                className: q.classes?.map((c) => c.class?.name).filter(Boolean).join(', ') || 'Kelas',
