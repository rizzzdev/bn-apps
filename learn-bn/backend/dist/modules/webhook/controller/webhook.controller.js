    ...
    syncAll = async (req, res, next) => {
        try {
            ...
            for (const mod of masterModules) {
                try {
                    const response = await fetch(`${masterApi}/api/v1/webhook/${mod}/sync`, {
                        method: 'POST',
            ...
            for (const mod of akademikModules) {
                try {
                    const response = await fetch(`${akademikApi}/api/v1/webhook/${mod}/sync`, {
                        method: 'POST',
                        headers: { 'x-api-key': apiKey }
                    });
                    const payload = await response.json();
