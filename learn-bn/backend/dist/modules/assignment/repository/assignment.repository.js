...
export class AssignmentRepository {
    ...
    async findAllByTeacher(teacherId) {
        ...
        return assignments.map((a) => ({
            ...
            submissionStats: {
                ...
                totalStudents: a.classes?.reduce((sum, c) => sum + (c.class?._count?.classStudents ?? 0), 0) ?? 0,
            },
        }));
    }
    async findAllByClass(classId, isStudent = false, studentId) {
        ...
        return assignments.map((a) => {
            ...
            const totalStudents = a.classes?.reduce((sum, c) => sum + (c.class?._count?.classStudents ?? 0), 0) ?? 0;
            if (studentId) {
