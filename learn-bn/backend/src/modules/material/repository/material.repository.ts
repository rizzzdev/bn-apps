...
export class MaterialRepository {
  async create(data: CreateMaterialDto, teacherId: string) {
    const targetClassIds =
      data.classIds && data.classIds.length > 0 ? data.classIds : data.classId ? [data.classId] : [];
  ...
  async findAllByClass(classId: string, isStudent = false, studentId?: string) {
    ...
    return materials.map((m: any) => {
      ...
      const totalStudents = m.classes?.reduce((sum: number, c: any) => sum + (c.class?._count?.classStudents ?? 0), 0) ?? 0;
  ...
  async findById(id: string, studentId?: string) {
    ...
    if (!studentId && material.classes && material.classes.length > 0) {
      const classIds = m.classes.map((c: any) => c.classId || c.class?.id).filter(Boolean);
      ...
      const readMap = new Map((m.reads || []).map((r: any) => [r.student?.id || r.studentId, r]));

      const readers = classStudents.map((cs) => {
        const readRecord: any = readMap.get(cs.student.id);
        return {
          student: cs.student,
          class: cs.class,
          isRead: !!readRecord,
  ...
  async update(id: string, data: UpdateMaterialDto) {
    const targetClassIds =
      data.classIds !== undefined ? data.classIds : data.classId ? [data.classId] : undefined;
