<script lang="ts">
	...
	interface AssignmentWithMySub extends Assignment {
		submission?: AssignmentSubmission | null;
	}

	let assignments = $state<AssignmentWithMySub[]>([]);
	...
	async function loadAssignments() {
		...
		try {
			...
			const publishedAssignments = res.filter((a) => a.status === 'Published');

			const withSubmissions = await Promise.all(
				publishedAssignments.map(async (a) => {
					const submission = await lmsStore.getMySubmission(a.id);
					return { ...a, submission, isSubmitted: !!submission };
				})
			);
			assignments = withSubmissions;
	...
	let statusOptions: SelectOption[] = [
		{ label: 'Semua Status Pengerjaan', value: 'all' },
		{ label: 'Sudah Dikerjakan', value: 'submitted' },
		{ label: 'Belum Dikerjakan', value: 'unsubmitted' }
	];

	let filteredAssignments = $derived(
		assignments.filter((a) => {
			if (selectedStatus === 'submitted' && !a.submission) return false;
			if (selectedStatus === 'unsubmitted' && a.submission) return false;
...
<div class="flex flex-col gap-6 mt-2">
	...
	<div class="bg-surface-container neo-border p-4 grid grid-cols-1 sm:grid-cols-2 gap-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
		...
		/>
		<CustomSelect
			label="Filter Status Pengerjaan"
