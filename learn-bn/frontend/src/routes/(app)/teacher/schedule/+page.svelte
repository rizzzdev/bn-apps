<script lang="ts">
	...
	let schedulePromise = $derived(
		authState.user?.profileId
			? lmsStore.getSchedule(authState.user.profileId)
			: lmsStore.getSchedule()
	);
</script>

<svelte:head>
	<title>Jadwal Guru - Akademik-BN</title>
</svelte:head>
...
<div class="flex flex-col gap-stack-md">
	...
	{:then schedule}
		<ScheduleGrid {schedule} type="teacher" />
	{:catch error}
		<div class="bg-surface-container-lowest neo-border shadow-[4px_4px_0px_0px_rgba(26,28,28,1)] p-12 text-center flex flex-col items-center gap-4">
