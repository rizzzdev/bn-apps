<script lang="ts">
	...
	// Set Homeroom Modal (like major's Set Head)
...
<div class="flex flex-col gap-8">
	...
	{:else}
		<ClassTable
			{classes}
			onView={openView}
			onAddStudent={openAddStudent}			onSetHomeroom={openSetHomeroom}
...
<Modal bind:isOpen={isAddStudentOpen} title="Tambah Pemetaan Murid Kelas">
	<div class="flex flex-col gap-4">
		...
		<SearchableSelect
			...
			options={allStudents
				.filter((s) => s.status === 'Aktif' && (selectedClassMajorId ? majorStudentMap[s.id] === selectedClassMajorId : true))
...
<Modal bind:isOpen={isSetHomeroomOpen} title={`Set Wali Kelas - ${targetHomeroomClass?.name ?? ''}`}>
	<div class="flex flex-col gap-4">
		<SearchableSelect
			id="teacher-select"
