...
<div class="flex flex-col gap-2 relative {className}" bind:this={containerRef}>
	...
	<button
		...
		{id}
		onclick={toggleOpen}
		class="w-full bg-surface-container-lowest neo-border px-4 py-3 font-data-mono text-data-mono text-on-background focus:outline-none focus:shadow-[4px_4px_0px_0px_#1C1B1B] transition-shadow text-left flex justify-between items-center"
	>
		<span
		...
		>
				...
				<div class="p-2 bg-surface-container-lowest neo-border-b z-10 shrink-0">
					<div class="flex items-center neo-border px-3 py-2 bg-surface">
						...
						<input
							...
							placeholder="Cari..."
							class="w-full bg-transparent border-none outline-none font-data-mono text-sm"
			...
			{#if multiple}
				<div class="flex border-b-2 border-on-surface bg-surface z-10 shrink-0">
					<button
						type="button"
						class="flex-1 py-2 font-data-mono text-xs font-bold text-center border-r-2 border-on-surface bg-surface text-primary hover:bg-primary hover:text-on-primary transition-colors"
						onclick={(e) => {
							e.stopPropagation();
					...
					<button
						type="button"
						class="flex-1 py-2 font-data-mono text-xs font-bold text-center bg-surface text-error hover:bg-error hover:text-on-error transition-colors"
						onclick={(e) => {
							e.stopPropagation();
							value = [];
						}}
				...
				{#if Array.isArray(value) && value.length > 0}
					...
					>
						{#each value as v}
							...
							{#if opt}
								<span
									class="bg-secondary text-on-secondary text-[10px] px-2 py-1 neo-border-xs whitespace-nowrap"
									>{opt.label}</span
								>
							{/if}
						{/each}
			...
			<div class="p-1 flex flex-col max-h-64 overflow-y-auto">
				{#each filteredOptions as opt}
					...
					<button
						type="button"
						class="text-left px-4 py-2 hover:bg-primary hover:text-on-primary transition-colors font-data-mono text-sm flex items-center justify-between {isSelected
