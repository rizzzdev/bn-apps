...
export const classSubjectRequirementApi = {
	...
	upsert: (data: ClassSubjectRequirement) =>
		createItem<ClassSubjectRequirement, ClassSubjectRequirement>(
...
export const teacherUnavailabilityApi = {
	list: (teacherId?: string) =>
		getList<TeacherUnavailability>(
			'/academic/teacher-unavailabilities',
			teacherId ? { teacherId } : undefined
		),
	bulkSet: (
		teacherId: string,
		unavailabilities: { day: string; lessonHourId: string; reason?: string }[]
	) =>
		createItem<
			{
				teacherId: string;
			...
			TeacherUnavailability[]
		>('/academic/teacher-unavailabilities/batch', { teacherId, unavailabilities }),
...
export type GeneratePreviewResponse =
	{ mode: 'queue'; jobId: string } | { mode: 'inline'; result: GeneratorPreviewResult };
...
export const timetableGeneratorApi = {
	preview: (options: GenerateOptions) =>
		createItem<GenerateOptions, GeneratePreviewResponse>(
			'/academic/lesson-schedules/generator/preview',
			options
		),
	...
	commit: (payload: CommitPayload) =>
		createItem<CommitPayload, { count: number }>(
			'/academic/lesson-schedules/generator/commit',
			payload
		)
};
