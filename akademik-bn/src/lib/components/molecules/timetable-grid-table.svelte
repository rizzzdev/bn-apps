<script lang="ts">
	...
	function handleSlotClick(slot: TimetableCellSlot, e: Event) {
	...
	<table class="w-full border-collapse font-data-mono text-data-mono">
		<thead>
			<tr class="neo-border-b bg-surface-container">
				<th
					class="p-3 text-left font-label-caps text-label-caps uppercase text-on-surface-variant border-r-2 border-on-surface min-w-[120px]"
				>
					Jam
				</th>
				{#each days as day}
					<th
						class="p-3 text-left font-label-caps text-label-caps uppercase text-on-surface-variant border-r-2 border-on-surface last:border-r-0 min-w-[200px]"
					>
						{day}
					</th>
				{/each}
		...
		<tbody>
			{#each sortedHours as lh}
				<tr class="neo-border-b last:border-b-0">
					<td
						class="p-3 font-bold text-on-surface-variant border-r-2 border-on-surface align-top whitespace-nowrap text-xs leading-tight"
					>
						{lh.name}<br />
										...
										class="neo-border-sm p-2 mb-2 last:mb-0 transition-shadow hover:shadow-[2px_2px_0px_0px_#1C1B1B] {slot.isEvent
											...
											: slot.colorClass
												...
												: selected
													...
													: 'bg-surface'} {onSlotClick || (selectable && !slot.isEvent)
											? 'cursor-pointer'
											: ''}"
										onclick={(e) => handleSlotClick(slot, e)}
										onkeydown={(e) => e.key === 'Enter' && handleSlotClick(slot, e)}
										role="button"
										tabindex="0"
									>
											...
											<div class="min-w-0 flex-1">
												...
												{#if slot.isEvent && showEventClassLabel}
													<div class="mt-1 flex flex-wrap gap-1">
														<span
															class="bg-surface text-xs px-1.5 py-0.5 neo-border-xs whitespace-nowrap flex items-center gap-1"
														>
												...
												{#if slot.teachers && slot.teachers.length > 0}
													<div class="mt-1 flex flex-wrap gap-1">
														{#each slot.teachers as t}
															<span
																class="bg-secondary-fixed-dim text-xs px-1.5 py-0.5 neo-border-xs whitespace-nowrap flex items-center gap-1"
															>
												...
												{#if slot.classes && slot.classes.length > 0}
													<div class="mt-1 flex flex-wrap gap-1">
														{#each slot.classes as c}
															<span
																class="bg-tertiary-fixed-dim text-xs px-1.5 py-0.5 neo-border-xs whitespace-nowrap flex items-center gap-1"
															>
												...
												{#if slot.notes}
													<p class="text-xs text-on-surface-variant mt-1 italic truncate">
														📝 {slot.notes}
													</p>
												{/if}
											</div>
