<script lang="ts">
  ...
  async function handleSave() {
    if (!formName) { toast.error('Nama jam pelajaran wajib diisi'); return; }
    if (formStartTime >= formEndTime) { toast.error('Jam mulai harus sebelum jam selesai'); return; }
...
<div class="flex flex-col gap-6">
  ...
  <div class="flex flex-row items-center justify-end gap-2">
	...
	<DataTable {isLoading} {error} isEmpty={lessonHours.length === 0} emptyMessage="Belum ada jam pelajaran">
		{#snippet children()}
...
<Modal bind:isOpen={isFormOpen} title={editingId ? 'Edit Jam Pelajaran' : 'Tambah Jam Pelajaran'}>
  <div class="flex flex-col gap-4">
    <div class="flex flex-col gap-1">
      <label class="font-label-caps text-xs uppercase font-bold text-on-surface-variant" for="name">Nama Jam</label>
      <input
        id="name"
        type="text"
        bind:value={formName}
    ...
    <div class="grid grid-cols-2 gap-3">
      <div class="flex flex-col gap-1">
        <label class="font-label-caps text-xs uppercase font-bold text-on-surface-variant" for="start-time">Jam Mulai</label>
        <input
          id="start-time"
          type="time"
      ...
      <div class="flex flex-col gap-1">
        <label class="font-label-caps text-xs uppercase font-bold text-on-surface-variant" for="end-time">Jam Selesai</label>
        <input
          id="end-time"
          type="time"
    ...
    <div class="flex flex-col gap-1">
      <label class="font-label-caps text-xs uppercase font-bold text-on-surface-variant" for="order">Urutan</label>
      <input
        id="order"
        type="number"
