<script lang="ts">
	...
	import type { ShadowClass, ShadowStudent, ShadowAcademicYear } from '$lib/types';
	...
	onMount(async () => {
		...
		try {
			const [classRes, csRes, studRes, ayRes] = await Promise.all([
				classApi.getById(id),
			...
			const nonActive = classStudents.filter(
				(cs) => cs.classId === id && !cs.deletedAt && cs.status !== 'Aktif'
			);
			...
			const list = Array.from(studentMap.values()).map((cs) => {
				const student = allStudents.find((s) => s.id === cs.studentId);
				const ay = ays.find((a) => a.id === cs.academicYearId);
				return {
					id: cs.id,
					studentId: cs.studentId,
					fullname: student?.fullname ?? cs.studentId,
					nis: student?.nis ?? '-',
					nisn: student?.nisn ?? '-',
					academicYearCode: ay ? `${ay.code} (${ay.semesterType})` : cs.academicYearId,
					status: cs.status,
					updatedAt: new Date(cs.updatedAt || cs.createdAt).toLocaleDateString('id-ID', {
						day: '2-digit',
						month: 'short',
						year: 'numeric'
					}),
					rawDate: new Date(cs.updatedAt || cs.createdAt).getTime()
				};
			});
...
<svelte:head>
	<title>{cls ? `Riwayat Alumni ${cls.name} - Akademik-BN` : 'Riwayat Alumni Kelas - Akademik-BN'}</title>
</svelte:head>

<div class="flex flex-col gap-8">
	<PageHeader
		title="Riwayat Alumni & Murid Non-Aktif"
		description={cls ? `ALUMNI // ${cls.name}` : 'ALUMNI'}
	/>
						...
						<div class="flex items-center gap-2">
							...
							<Badge
								variant={item.status === 'Aktif'
									...
									: item.status === 'Lulus'
										? 'success'
										: item.status === 'Pindah' || item.status === 'Naik Kelas' || item.status === 'Tinggal Kelas'
											? 'warning'
											: 'default'}
							>
								{item.status}
