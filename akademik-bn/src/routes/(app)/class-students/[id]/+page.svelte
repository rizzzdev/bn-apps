<script lang="ts">
	...
	import type { Class, ShadowClass, ShadowStudent, ShadowAcademicYear, MajorStudent, ShadowTeacher, HomeroomTeacher } from '$lib/types';
	...
	let filteredAddStudents = $derived(
		classMajorId
			? availableAddStudents.filter(
					(s) => majorStudentMap[s.id] === classMajorId
				)
...
<div class="flex flex-col gap-6">
	...
	{:else if error}
		<div class="neo-border bg-error-container text-error p-4 text-center font-data-mono text-xs">
			...
			<a href="/class-students" class="font-data-mono text-xs text-primary underline mt-4 inline-block">
