<script lang="ts">
	...
	let createBatchWeeklyHours = $state(0);
	let createBatchGroupId = $state('');
	...
	function openCreateMapping() {
		...
		createBatchWeeklyHours = 0;
		createBatchGroupId = crypto.randomUUID();
		createTeacherIds = [];
			...
			? previewResult.schedules
					...
					.map((s) => {
						const teacherList = s.teacherIds
							? s.teacherIds.map((tid, i) => ({
									id: tid,
									name: (s.teacherNames || [])[i] || s.teacherName
								}))
							: [{ id: s.teacherId, name: s.teacherName }];
						const classList = s.classIds
							? s.classIds.map((cid, i) => ({
									id: cid,
									name: (s.classNames || [])[i] || s.className
								}))
	...
	function checkOverAllocatedTeachers(): boolean {
		...
		for (const tId of assignedTeacherIds) {
			...
			const stRecords = subjectTeachers.filter(
				(st) => st.teacherId === tId && st.status === 'Aktif'
			);
			...
			for (const st of stRecords) {
				...
				if (allocatedForSubject > targetForSubject) {
					toast.error(
						`Guru ${formatTeacherName(teacherObj)} melebihi batas beban mengajar per kelas! (Dialokasikan: ${allocatedForSubject} JP, Target Beban: ${targetForSubject} JP)`
					);
					return true;
				}
			}
			...
			if (totalAllocated > totalTarget) {
				toast.error(
					`Guru ${formatTeacherName(teacherObj)} melebihi total batas beban mengajar! (Total Dialokasikan: ${totalAllocated} JP, Total Target Beban: ${totalTarget} JP)`
				);
				return true;
			}
		}
	...
	async function handleCreateMapping() {
		...
		if (createTeacherIds.length === 0) {
			toast.error('Pilih minimal satu guru pengampu');
			return;
		}
		if (createClassIds.length === 0) {
			toast.error('Pilih minimal satu kelas');
			return;
		}
		if (createBatchWeeklyHours > createWeeklyHours) {
			toast.error('JP batch tidak boleh melebihi total JP mingguan');
			return;
		}
		if (createBatchWeeklyHours > 0 && createClassIds.length < 2) {
			toast.error('Batch teaching membutuhkan minimal 2 kelas');
			return;
		}
		...
		try {
			...
			for (const cId of createClassIds) {
				for (const tId of createTeacherIds) {
					payloads.push({
						...
						batchWeeklyHours: createBatchWeeklyHours,
						batchGroupId: createBatchWeeklyHours > 0 ? createBatchGroupId : null,
			...
			const msg = e?.response?.data?.message || e?.message || 'Gagal membuat mapping';
			toast.error(msg);
		} finally {
	...
	let createTeacherOptions = $derived(
		createSubjectId
			? [
					...
					...getTeachersForSubject(createSubjectId).map((t) => ({
						value: t.id,
	...
	async function loadRequirementsForClass(classId: string) {
		...
		try {
			...
			const merged: SubjectRequirementRow[] = eligibleSubjects.map((sub) => {
				...
				const reqTeacherIds = g
					? g.rows.map((r) => r.teacherId).filter((t): t is string => t !== null && t !== undefined)
					: [];
				...
				const batchWeeklyHours = g ? Math.max(...g.rows.map((r) => r.batchWeeklyHours || 0)) : 0;
	...
	// Refresh seluruh data alokasi (dipakai setelah import Excel).
	...
	function selectAllReqs() {
		const savedIds = allClassRequirements
			.map((r) => r.id)
	...
	async function runGenerator() {
		...
		try {
			...
			if (!data) {
				toast.error(
					'Generator tidak menghasilkan data. Pastikan matriks Beban Jam (Tab 1) sudah terisi.'
				);
				return;
			}
	...
	async function handleExportRequirements() {
		try {
			const date = new Date().toISOString().slice(0, 10);
			await downloadExcel(
				'/academic/class-subject-requirements/export',
				`matriks-alokasi-jam_${date}.xlsx`
			);
			toast.success('Data matriks alokasi jam berhasil diunduh');
		} catch {
			toast.error('Gagal mengunduh data matriks alokasi jam');
		}
	}

	async function handleExportUnavailabilities() {
		try {
			const date = new Date().toISOString().slice(0, 10);
			await downloadExcel(
				'/academic/teacher-unavailabilities/export',
				`halangan-guru_${date}.xlsx`
			);
			toast.success('Data halangan guru berhasil diunduh');
		} catch {
			toast.error('Gagal mengunduh data halangan guru');
		}
	}
</script>
...
<div class="flex flex-col gap-6 font-data-mono text-data-mono min-w-0 max-w-full">
	...
	<div class="neo-border bg-surface p-2 grid grid-cols-2 lg:grid-cols-4 gap-2">
		<button
			...
			class="p-3 text-left font-label-caps text-label-caps uppercase neo-border transition-all flex items-center gap-2.5 {activeTab ===
			'requirements'
				? 'bg-primary text-on-primary neo-shadow-sm font-bold'
				: 'bg-surface hover:bg-surface-container text-on-surface'}"
		>
			<span
				class="w-6 h-6 neo-border bg-surface text-on-surface text-xs flex items-center justify-center font-bold"
				>1</span
			>
		...
		<button
			...
			class="p-3 text-left font-label-caps text-label-caps uppercase neo-border transition-all flex items-center justify-between gap-2 {activeTab ===
			'preview'
				? 'bg-primary text-on-primary neo-shadow-sm font-bold'
				: 'bg-surface hover:bg-surface-container text-on-surface'}"
		>
			...
			{#if previewResult}
				<span
					class="px-2 py-0.5 text-xs font-bold neo-border bg-tertiary-fixed text-on-tertiary-fixed"
				>
	...
	{:else}
		...
		{#if activeTab === 'requirements'}
			<div class="neo-border bg-surface p-6 flex flex-col gap-6">
				<div
					class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 neo-border-b pb-4"
				>
					<div>
						...
						<p class="text-xs text-on-surface-variant font-data-mono uppercase">
							Data beban jam per kelas (read-only). Tambah mapping baru lewat form &quot;Buat
							Mapping Baru&quot;.
						</p>
					</div>
					<div class="flex items-center gap-3 w-full sm:w-auto">
						...
						<div class="mt-5 flex items-center gap-2">
							...
							/>
							<ExcelImport
								templateEndpoint="/academic/class-subject-requirements/template"
								templateFilename="class_subject_requirements_template.xlsx"
								uploadEndpoint="/academic/class-subject-requirements/batch/excel"
								serviceLabel="Alokasi Jam"
								onSuccess={refreshRequirements}
							/>
							<TooltipIconButton
								icon="download"
								tooltip="Export Matriks Alokasi"
						...
						<thead>
							...
							>
								<th class="p-3 border-r-2 border-on-surface w-10">
									<Checkbox
										checked={selectedReqIds.length > 0 &&
											selectedReqIds.length ===
												allClassRequirements.filter((r) => Boolean(r.id)).length}
										onchange={selectAllReqs}
									/>
								</th>
						...
						<tbody>
							{#if displayedRequirements.length === 0}
								<tr>
									<td
										colspan="5"
										class="p-10 text-center text-on-surface-variant font-data-mono text-data-mono uppercase"
									>
										Belum ada data alokasi jam pelajaran untuk kelas ini. Gunakan tombol &quot;Buat
										Mapping Baru&quot; di atas untuk menambahkan data Beban Jam (dengan guru
										pengampu terpilih).
									</td>
								</tr>
							{:else}
									...
									<tr class="neo-border-b hover:bg-surface-container-low">
										<td class="p-3 border-r-2 border-on-surface text-center">
											{#if req.existingIds.length > 0}
												<Checkbox
													checked={req.existingIds.some((id) => selectedReqIds.includes(id))}
													onchange={() => toggleReqSelect(req.existingIds)}
												/>
											{:else}
										...
										<td class="p-3 border-r-2 border-on-surface font-bold text-on-surface">
											{req.subjectName}
											<span class="text-xs text-on-surface-variant block font-normal"
												>{req.subjectCode}</span
											>
										</td>
										<td class="p-3 border-r-2 border-on-surface text-center font-bold">
											{req.weeklyHours}
											<span class="text-xs font-normal text-on-surface-variant">JP</span>
											{#if req.batchWeeklyHours > 0}
												<span
													class="block text-[10px] font-bold mt-0.5 px-1.5 py-0.5 neo-border-xs bg-secondary-container text-on-secondary-container whitespace-nowrap"
												>
													{req.batchWeeklyHours} JP BATCH
												</span>
											{/if}
										</td>
										<td class="p-3 border-r-2 border-on-surface text-center font-bold">
			...
			<div class="neo-border bg-surface p-6 flex flex-col gap-6">
				...
				>
					<div>
						...
						<p class="text-xs text-on-surface-variant font-data-mono uppercase">
							Klik pada slot jam/hari untuk menandai waktu libur/halangan mengajar guru (Merah =
							Halangan).
						</p>
					</div>
					<div class="flex items-center gap-3 w-full sm:w-auto">
						...
						<div class="mt-5 flex items-center gap-2">
							...
							/>
							<TooltipIconButton
								icon={selectedUnavailHourIds.length > 0 &&
								selectedUnavailHourIds.length ===
									sortedLessonHours.filter((h) =>
										Array.from(blockedSlotKeys).some((k) => k.endsWith(`_${h.id}`))
									).length &&
								sortedLessonHours.some((h) =>
									Array.from(blockedSlotKeys).some((k) => k.endsWith(`_${h.id}`))
								)
									? 'deselect'
									: 'checklist'}
								tooltip={selectedUnavailHourIds.length > 0
									? 'Batal Pilih Semua'
									: 'Pilih Semua Halangan'}
								onclick={selectAllUnavailHours}
							/>
							<ExcelImport
								templateEndpoint="/academic/teacher-unavailabilities/template"
								templateFilename="teacher_unavailabilities_template.xlsx"
								uploadEndpoint="/academic/teacher-unavailabilities/batch/excel"
								serviceLabel="Halangan Guru"
								onSuccess={async () => {
									if (selectedTeacherId) await loadTeacherUnavailabilities(selectedTeacherId);
								}}
							/>
							<TooltipIconButton
								icon="download"
								tooltip="Export Halangan Guru"
								onclick={handleExportUnavailabilities}
							/>
					...
					<table class="w-full text-center font-data-mono text-data-mono border-collapse">
						<thead>
							...
							>
								<th class="p-3 border-r-2 border-on-surface w-10">
									<Checkbox
										checked={selectedUnavailHourIds.length > 0 &&
											selectedUnavailHourIds.length ===
												sortedLessonHours.filter((h) =>
													Array.from(blockedSlotKeys).some((k) => k.endsWith(`_${h.id}`))
												).length}
										onchange={selectAllUnavailHours}
									/>
								</th>
								...
								<tr class="neo-border-b">
									<td class="p-2 border-r-2 border-on-surface text-center">
										{#if Array.from(blockedSlotKeys).some((k) => k.endsWith(`_${hour.id}`))}
											<Checkbox
												checked={selectedUnavailHourIds.includes(hour.id)}
												onchange={() => toggleUnavailHourSelect(hour.id)}
											/>
										{:else}
											<span class="text-on-surface-variant text-xs">—</span>
										{/if}
									...
									<td
										class="p-2 border-r-2 border-on-surface font-bold bg-surface-container-low text-on-surface"
									>
										{hour.name}
										<span class="block text-[10px] text-on-surface-variant font-normal"
											>{hour.startTime}-{hour.endTime}</span
										>
									</td>
									{#each days as day}
		...
		{#if activeTab === 'run'}
			<div class="neo-border bg-surface p-8 max-w-2xl mx-auto flex flex-col gap-6 neo-shadow">
				...
				<div class="flex flex-col gap-4">
					<div>
						<span
							class="block text-xs font-label-caps text-label-caps uppercase text-on-surface mb-2"
						>
							Hari Kerja Mengajar:
						</span>
						<div class="flex flex-wrap gap-2">
							{#each ['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'] as d}
								<button
									...
									class="px-4 py-2 neo-border font-label-caps text-label-caps uppercase transition-all {selectedWorkingDays.includes(
										d
									)
					...
					<div>
						<label
							for="timeoutInput"
							class="block text-xs font-label-caps text-label-caps uppercase text-on-surface mb-1"
						>
						...
						/>
						<span class="text-[10px] text-on-surface-variant uppercase mt-1 block"
							>Default: 15000 ms (15 detik)</span
						>
					</div>
					...
					<div class="flex items-center gap-3">
						...
						/>
						<div>
							<span class="font-label-caps text-label-caps uppercase text-on-surface"
								>Batch Teaching</span
							>
							<p class="text-[10px] text-on-surface-variant">
								Gabungkan kelas yang ditandai &quot;JP Batch&quot; (batchGroupId sama) ke satu slot.
								Nonaktif = semua dijadwalkan individual
							</p>
						</div>
					</div>
				</div>
				...
				<div class="pt-4 neo-border-t">
					...
					>
						{#if isGenerating}
							<div
								class="animate-spin rounded-full h-5 w-5 border-2 border-on-primary border-t-transparent shrink-0"
							></div>
					...
					{#if isGenerating}
						<p class="text-[10px] text-on-surface-variant uppercase mt-2 text-center">
							Proses berjalan di background — biarkan halaman ini terbuka; hasil akan muncul
							otomatis.
						</p>
					{/if}
				</div>
			...
			{:else}
				...
				<div class="grid grid-cols-1 sm:grid-cols-4 gap-4 font-data-mono text-data-mono">
					<div class="neo-border bg-surface p-4 neo-shadow-sm">
						<span class="text-xs font-label-caps uppercase text-on-surface-variant"
							>Skor Kualitas Jadwal</span
						>
						<div class="text-3xl font-black text-on-surface mt-1">
							{previewResult.qualityScore}%
						</div>
						<span class="text-[10px] text-on-surface-variant uppercase block mt-1"
							>0 Hard Conflict</span
						>
					</div>

					<div class="neo-border bg-surface p-4 neo-shadow-sm">
						<span class="text-xs font-label-caps uppercase text-on-surface-variant"
							>Beban Jam Teralokasi</span
						>
						<div class="text-3xl font-bold text-on-surface mt-1">
							{previewResult.stats.assignedHours} / {previewResult.stats.totalHours}
							<span class="text-xs font-normal text-on-surface-variant">JP</span>
						</div>
					</div>
					...
					<div class="neo-border bg-surface p-4 neo-shadow-sm">
						<span class="text-xs font-label-caps uppercase text-on-surface-variant"
							>Langkah Iterasi (Attempts)</span
						>
						<div class="text-3xl font-bold text-on-surface mt-1">
							{previewResult.stats.attempts.toLocaleString()}
						</div>
					</div>
					...
					<div class="neo-border bg-surface p-4 neo-shadow-sm">
						...
						>
						<div class="text-3xl font-bold text-on-surface mt-1">
							{previewResult.stats.durationMs}
							<span class="text-xs font-normal text-on-surface-variant">ms</span>
						</div>
					</div>
				</div>
				...
				{#if previewResult.unassigned.length > 0}
					<div
						class="neo-border bg-warning-container text-on-warning-container p-4 flex flex-col gap-2 font-data-mono text-data-mono"
					>
						<div class="flex items-center gap-2 font-bold text-sm uppercase">
							<Icon name="warning" />
							<span
								>Ada {previewResult.unassigned.length} Pelajaran Yang Belum Teralokasi (Partial Schedule)</span
							>
						</div>
						<ul class="text-xs space-y-1 list-disc list-inside uppercase">
							{#each previewResult.unassigned as item}
								<li>
									Kelas <span class="font-bold">{item.classNames.join(', ')}</span> - {item.subjectName}
									({item.teacherNames.join(', ')}) [{item.duration} JP]: {item.reason}
								</li>
							{/each}
						</ul>
					</div>
...
<Modal bind:isOpen={isPublishModalOpen} title="Publikasikan Jadwal Pelajaran">
	<div class="flex flex-col gap-4 font-data-mono text-data-mono">
		<p class="text-sm text-on-surface">
			Apakah Anda yakin ingin mempublikasikan draf hasil jadwal ini? Jadwal pelajaran yang aktif
			saat ini di database akan diperbarui.
		</p>

		<div class="neo-border p-4 bg-surface-container-low text-xs text-on-surface space-y-1">
			<div>
				Total slot jadwal: <span class="font-bold">{previewResult?.schedules.length || 0}</span>
			</div>
			<div>
				Skor kelayakan: <span class="font-bold text-emerald-600"
					>{previewResult?.qualityScore || 0}%</span
				>
			</div>
		</div>
...
<Modal bind:isOpen={isBulkDeleteUnavailOpen} title="Konfirmasi Hapus Massal Halangan Guru">
	<p class="font-body-md text-body-md">
		Yakin ingin menghapus halangan mengajar guru untuk <strong
			>{selectedUnavailHourIds.length}</strong
		> jam pelajaran yang terpilih?
	</p>
	{#snippet footer()}
...
<Modal bind:isOpen={isClearAllReqOpen} title="Konfirmasi Hapus Semua Alokasi Kelas">
	<div class="flex flex-col gap-3">
		...
		<div class="neo-border p-3 bg-error-container text-error text-xs font-data-mono">
			...
			matriks beban jam untuk kelas ini. Data yang dihapus bersifat soft-delete dan dapat dipulihkan
			dari database.
		</div>
	</div>
	{#snippet footer()}
...
<Modal bind:isOpen={isCreateMappingOpen} title="Buat Mapping Baru">
	<div class="flex flex-col gap-4 overflow-y-auto max-h-[55dvh] pr-1">
		...
		<div>
			<label
				for="create-map-batch"
				class="font-label-caps text-label-caps uppercase text-on-surface-variant"
				>JP Batch (Diajarkan Bersama)</label
			>
			<input
				id="create-map-batch"
				type="number"
				min="0"
				bind:value={createBatchWeeklyHours}
				class="w-full h-11 px-3 neo-border bg-surface text-on-surface font-data-mono font-bold text-center mt-1"
			/>
			<span class="text-[10px] text-on-surface-variant uppercase mt-1 block">
				JP ini dijadwalkan bersama untuk semua kelas terpilih (minimal 2 kelas)
			</span>
		</div>
		<SearchableSelect
