...
<div class="flex flex-col gap-md">
	<Card title="Informasi Personal">
		<div class="flex flex-col items-center gap-md">
			...
			<div class="w-full flex justify-center">
				...
				{:else}
					<div
						class="h-48 w-48 md:h-56 md:w-56 border-4 border-on-background rounded-2xl shadow-neo-lg flex items-center justify-center bg-linear-to-br from-secondary to-primary"
					>
						<span
							class="text-5xl md:text-6xl font-headline-md text-on-secondary font-bold tracking-tighter"
						>
							{getInitials()}
						</span>
					</div>
			...
			<div class="w-full flex flex-col gap-sm mt-sm">
				...
				<DetailRow label="Kelas Saat Ini" value={student.currentClass?.name || null} />
				<DetailRow label="Jurusan" value={student.currentMajor?.name || null} />
				...
				<DetailRow label="Golongan Darah" value={student.bloodType} />
	...
	<Card title="Data Kontak">
		<div class="flex flex-col gap-sm">
			...
			<DetailRow label="No. HP Orang Tua" value={student.parentPhone} />
		</div>
	</Card>
</div>
