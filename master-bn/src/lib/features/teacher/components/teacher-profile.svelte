<script lang="ts">
	...
	const getPictureSrc = () => {
		if (teacher.picture?.url) {
			return `${PUBLIC_API_URL}/master/attachments/file/${teacher.picture.url}`;
		}
		return null;
	};
			...
			<div class="w-full flex justify-center">
				{#if getPictureSrc()}
					<Avatar
						...
						size="h-32 w-32 md:h-36 md:w-36	"
						rounded="rounded-2xl"
						shadow="shadow-neo-lg"
					/>
				{:else}
					<div
						class="h-32 w-32 md:h-36 md:w-36 border-4 border-on-background rounded-2xl shadow-neo-lg flex items-center justify-center bg-linear-to-br from-secondary to-primary"
					>
						<span
							class="text-3xl md:text-4xl font-headline-md text-on-secondary font-bold tracking-tighter"
						>
							{getInitials()}
						</span>
					</div>
