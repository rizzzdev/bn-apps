<script lang="ts">
	...
	let currentAccept = $derived(
		accept ||
			(type === 'image' || type === 'excel' || type === 'file' ? defaultAccept[type] : '') ||
			''
	);
	...
	let displaySrc = $derived(preview || existingImageUrl || null);
</script>
...
<div class="flex flex-col mb-sm w-full {className}">
	...
	{#if isFileType}
		<label
			for={id}
			class="border-3 border-dashed border-on-background rounded-lg p-md flex flex-col items-center justify-center bg-surface-container-lowest hover:bg-surface-variant transition-colors cursor-pointer relative overflow-hidden min-h-25"
		>
			<input
				{id}
				type="file"
			...
			{#if type === 'image'}
				{#if displaySrc}
					...
					<div class="relative inline-block">
						<img
							src={displaySrc}
							alt="Preview"
							class="h-32 w-32 object-cover rounded-2xl border-4 border-on-background shadow-neo-lg transition-all duration-200 {markedForDeletion
				...
				{:else}
					...
					<span class="font-label-sm text-label-sm text-on-surface-variant"
						>Pilih file untuk diunggah</span
					>
				{/if}
			{:else if file}
	...
	{:else}
		<div
			class="w-full h-8 relative border-3 border-on-background rounded-lg bg-surface px-sm py-1.5 transition-all flex items-center"
		>
