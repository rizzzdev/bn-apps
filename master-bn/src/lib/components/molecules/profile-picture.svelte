<script lang="ts">
	...
	let { pictureUrl, fullname, entityType, entityId, onUpdate } = $props<{
...
<div class="flex flex-col items-center gap-sm">
	...
	<div class="relative group">
		{#if mode === 'uploading'}
			<div
				class="h-28 w-28 border-3 border-on-background rounded-xl shadow-neo flex items-center justify-center bg-on-primary"
			>
				<Icon name="sync" class="text-3xl text-on-surface-variant animate-spin" />
			</div>
		{:else if getImageSrc()}
			<Avatar src={getImageSrc()!} alt={fullname} size="h-28 w-28" rounded="rounded-xl" />
		{:else}
			<div
				class="h-28 w-28 border-3 border-on-background rounded-xl shadow-neo flex items-center justify-center bg-secondary"
			>
	...
	{#if mode === 'preview'}
		<div class="flex gap-xs">
			<Button variant="primary" onclick={handleUpload}>Simpan</Button>
			<Button variant="secondary" onclick={cancelPreview}>Batal</Button>
		</div>
	{:else if !pictureUrl}
