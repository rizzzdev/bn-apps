...
<Modal bind:isOpen {title}>
	<div class="flex items-start gap-md">
		<div
			class="p-xs border-on-background border-3 rounded-lg shadow-neo-xs mt-1 {type === 'danger'
				? 'bg-error text-on-error'
				: type === 'warning'
					? 'bg-[#FCD34D] text-on-surface'
	...
	{#snippet footer()}
		...
		<Button
			variant={type === 'danger' ? 'error' : type === 'warning' ? 'warning' : 'info'}
