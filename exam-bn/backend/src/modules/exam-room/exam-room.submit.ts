...
export const forceSubmitAbsentParticipants = async (examRoomId: string): Promise<void> => {
  ...
  const answeredUserIds = Array.from(new Set(
    (
  ...
  if (allMC) {
    ...
    const toCreate = userScores.filter((s) => !existingUserIds.has(s.userId)).map(s => ({ examRoomId, ...s }));
    ...
    const toCreate = answeredUserIds.filter(id => !existingUserIds.has(id)).map(id => ({ examRoomId, userId: id, score: null }));
    if (toCreate.length > 0) {
      await prisma.examScore.createMany({ data: toCreate });
    }
