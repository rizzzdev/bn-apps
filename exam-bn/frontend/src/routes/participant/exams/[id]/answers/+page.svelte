...
<svelte:head>
	<title>Pembahasan - {data.examName}</title>
</svelte:head>

<div class="mb-6 flex items-center gap-2 text-sm text-(--text-secondary)">
	<a href="/participant/exams" class="hover:text-primary-500 transition-colors">Ujian Saya</a>
	<span>/</span>
	<span class="text-(--text-primary)">Pembahasan</span>
</div>

<div class="mb-6 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
	<div>
		<h1 class="text-2xl font-black text-(--text-primary)">Pembahasan Jawaban</h1>
		<p class="text-sm text-(--text-secondary) mt-1">{data.examName} &middot; {data.roomName}</p>
	</div>
...
{#if data.questions.length === 0}
	<div class="card p-10 text-center font-bold text-(--text-secondary)">Tidak ada data soal.</div>
{:else}
	<div class="space-y-4">
